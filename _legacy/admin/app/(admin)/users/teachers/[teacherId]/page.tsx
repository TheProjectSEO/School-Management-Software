...
async function getTeacherDetails(teacherId: string) {
  ...
  // Get real schedule from live_sessions table (if available)
  ...
  const schedule = (scheduleData || []).map((session) => {
    const course = session.courses as unknown as { name: string } | null;
    const section = session.sections as unknown as { name: string; grade_level: string } | null;
    return {
      day: dayNames[session.day_of_week] || "Unknown",
      time: `${session.start_time?.slice(0, 5) || ""} - ${session.end_time?.slice(0, 5) || ""}`,
      subject: course?.name || "Unknown Subject",
      section: section ? `Grade ${section.grade_level} - ${section.name}` : "Unknown Section",
    };
  });

  // Calculate real performance metrics
  // Get all section IDs this teacher teaches or advises
  const teacherSectionIds = [
    ...(sectionAssignments || []).map(a => a.sections && (a.sections as unknown as { id: string }).id).filter(Boolean),
    ...(courseAssignments || []).map(a => a.sections && (a.sections as unknown as { id: string }).id).filter(Boolean),
  ];
  const uniqueSectionIds = [...new Set(teacherSectionIds)];
  ...
  const avgClassSize = uniqueSectionIds.length > 0 ? Math.round(totalStudents / uniqueSectionIds.length) : 0;

  const metrics = {
    totalStudents,
    avgClassSize,
    sectionsCount: uniqueSectionIds.length,
    coursesCount: courseAssignments?.length || 0,
  };
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="text-center p-3 bg-purple-50 rounded-lg">
                <p className="text-2xl font-bold text-purple-600">{teacher.metrics.sectionsCount}</p>
                <p className="text-xs text-purple-600">Sections</p>
              </div>
              <div className="text-center p-3 bg-orange-50 rounded-lg">
                <p className="text-2xl font-bold text-orange-600">{teacher.metrics.coursesCount}</p>
                <p className="text-xs text-orange-600">Courses</p>
              </div>
            </div>
          </div>
        </div>
