...
export async function getGradeReport(params: {
  ...
  // If filtering by gradeLevel or courseId, first get matching enrollment IDs
  let enrollmentIds: string[] | null = null;

  if (gradeLevel || courseId) {
    let enrollmentQuery = supabase
      .from("enrollments")
      .select("id, section_id, course_id, sections!inner(grade_level)");

    if (gradeLevel) {
      enrollmentQuery = enrollmentQuery.eq("sections.grade_level", gradeLevel);
    }
    ...
    // If no matching enrollments, return empty result
  ...
  // Use enrollment IDs filter if we have grade level or course filters
