...
async function getTeacherDetails(teacherId: string) {
  ...
  // Generate schedule from actual course assignments
  const schedule: { day: string; time: string; subject: string; section: string }[] = [];

  if (courseAssignments && courseAssignments.length > 0) {
    courseAssignments.forEach((assignment, index) => {
      const course = assignment.courses as unknown as { name: string; subject_code: string };
  ...
  const avgClassSize = sectionCount > 0 ? Math.round(totalStudents / sectionCount) : 0;
  ...
  if (sessions && sessions.length > 0) {
    ...
    // Rough attendance calculation
    const expectedParticipants = sessions.length * avgClassSize;
    attendanceRate = expectedParticipants > 0
      ? Math.min(100, Math.round(((participantCount || 0) / expectedParticipants) * 100))
      : 95; // Default if no sessions
  } else {
    attendanceRate = 95; // Default if no sessions yet
  }
...
export default async function TeacherDetailPage({ params }: TeacherDetailPageProps) {
  ...
  const profile = teacher.school_profiles as unknown as { full_name: string; phone?: string; avatar_url?: string };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Link
            href={`/admin/users/teachers/${teacherId}/edit`}
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                ...
                <p className="text-gray-500">{teacher.employee_id || "No Employee ID"}</p>
              </div>
            </div>
        ...
        <div className="lg:col-span-2 space-y-6">
                        ...
                        <tr key={assignment.id}>
                          <td className="py-2 px-3">
                            <div>
                              ...
                              <p className="text-xs text-gray-500">{course?.subject_code}</p>
                            </div>
                          </td>
                          <td className="py-2 px-3">
                            {section && (/^grade\s/i.test(section.name.trim()) ? section.name : `Grade ${section.grade_level} - ${section.name}`)}
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-sm text-gray-500">Based on Course Assignments</span>
            </div>
