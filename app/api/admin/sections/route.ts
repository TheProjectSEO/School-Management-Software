...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase
      ...
      .eq("school_id", admin.schoolId) // Filter by admin's school
      .order("grade_level")
      .order("name");
    ...
    // Get enrollment counts and adviser info for each section
    const sectionsWithCounts = await Promise.all(
      (data || []).map(async (section) => {
        ...
        // Check if has adviser (handle missing table gracefully)
        ...
        try {
          ...
          // Table doesn't exist, assume no adviser
        }
