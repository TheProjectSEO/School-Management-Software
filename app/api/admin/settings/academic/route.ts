...
export async function GET(request: NextRequest) {
  try {
    ...
    if (type === "grading_scale") {
      const gradingScale = await getGradingScale(admin.schoolId);
      return NextResponse.json(gradingScale);
    }

    const settings = await getAcademicSettings(admin.schoolId);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (type === "grading_scale") {
      ...
      const result = await updateGradingScale(admin.schoolId, scales as GradingScale[]);
    ...
    const result = await updateAcademicSettings(admin.schoolId, updates);
