...
/**
 * Payload type for teacher_announcements table from Supabase realtime
 */
...
/**
 * SSE endpoint for real-time announcement streaming.
...
export async function GET(request: NextRequest) {
  ...
  if (!currentUser) {
    console.log("[SSE Announcements] Unauthorized: No JWT session");
    return new Response("Unauthorized", { status: 401 });
  }

  if (currentUser.role !== "student") {
    console.log("[SSE Announcements] Forbidden: Not a student role");
    return new Response("Forbidden - Student role required", { status: 403 });
  }

  const supabase = createServiceClient();

  // Get student profile and enrollment info for filtering using auth_user_id from JWT
      ...
      const channel = realtimeClient
        ...
        .on(
          ...
          (payload) => {
            ...
            // Cast payload.new and payload.old to the proper type
            const newAnnouncement = payload.new as AnnouncementPayload | null;
            const oldAnnouncement = payload.old as AnnouncementPayload | null;
            const announcement = newAnnouncement || oldAnnouncement;
