...
/**
 * Payload type for teacher_direct_messages table from Supabase realtime
 */
...
export async function GET(request: NextRequest) {
  ...
  if (!currentUser) {
    console.log("[SSE Messages] Unauthorized: No JWT session");
    return new Response("Unauthorized", { status: 401 });
  }

  if (currentUser.role !== "student") {
    console.log("[SSE Messages] Forbidden: Not a student role");
      ...
      const channel = realtimeClient
        ...
        .on(
          ...
          (payload) => {
            ...
            const oldMessage = payload.old as MessagePayload | null;
