...
export async function GET(request: NextRequest) {
  try {
    ...
    const gradingPeriodId = searchParams.get("gradingPeriodId") || searchParams.get("gradingPeriod") || undefined;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await exportReport("grades", format, filters);
    ...
    return new NextResponse(result.data, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="grades-report.${format === "excel" ? "xlsx" : format}"`,
      },
    });
  } catch (error) {
