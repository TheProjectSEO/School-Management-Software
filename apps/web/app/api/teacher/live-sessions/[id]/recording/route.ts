...
export async function GET(
  ...
  // Get session from public.live_sessions (primary) - use service client to bypass RLS
  const { data: studentSession, error: studentError } = await serviceClient
    .from('live_sessions')
...
export async function POST(
  ...
  const { data: studentSession } = await serviceClient
    .from('live_sessions')
