...
export async function getTeacherStats(teacherId: string): Promise<TeacherStats> {
  ...
  // Get unique section IDs from teacher_assignments (this is the correct source)
  ...
  const [
    ...
    getTotalStudentsCount(sectionIds),
...
export async function getRecentPendingSubmissions(
  ...
  return (data || []).map((submission: any) => ({
    id: submission.id,
    student_name: submission.student?.profile?.full_name || 'Unknown',
...
export async function getTodaysAbsentStudents(teacherId: string): Promise<AbsentStudent[]> {
  ...
  const sectionIds = assignments.map((a: any) => a.section?.id).filter(Boolean)
  ...
  for (const student of (students as any[])) {
    ...
    if (!attendance || attendance.status === 'absent') {
      absentStudents.push({
        id: student.id,
        full_name: student.profile?.full_name || 'Unknown',
        lrn: student.lrn || 'Unknown',
        section_name: student.section?.name || 'Unknown',
...
export async function getRecentActivity(
  ...
  // Get recent submissions (only if teacher has assessments)
    ...
    if (submissions) {
      submissions.forEach((sub: any) => {
        activities.push({
          id: sub.id,
          type: 'submission',
          description: `${sub.student?.profile?.full_name || 'Unknown'} submitted ${sub.assessment?.title || 'Unknown'}`,
...
export async function getSubjectsPageStats(teacherId: string): Promise<SubjectsPageStats> {
  ...
  const [
    ...
    // Total subjects = unique courses assigned to teacher
